...
class NGen:
			...
			for cfg_name in N.configs:
				...
				if cfg_name != DEFAULT_NAME and not cfg.is_target:
					...
					if parent_cfg.level >= 0:
						...
						cfg.copy_files = cfg.copy_files | default.copy_files
						cfg.suffix = cfg_name
	...
	def CopyFile(N, p, cfg):
		if os.path.isfile(p):
			#print(f" filename { xx } ");
			cfg.copy_files.add(p)
			print("COPY " + p)
		else:
			print("*** MISSING FILE FOR COPY %s\n" % p)
			exit(1)
	...
	def GetObjName(N, name, ext, cfg):
		...
		#if(archstr):
		#	archstr = "/" + archstr
		objname = f"$tempdir_{cfg.suffix}/{raw}"
		return objname, name
	...
	def WriteBuildFile(N):
		...
		with open("build.ninja", "w") as f:
			...
			for cfg_name in N.configs_ordered:
				cfg = N.configs[cfg_name]
				f.write(f"outdir_{cfg.name} = {os.path.join(N.outdir, N.active_platform, cfg.name)}\n")
				f.write(f"tempdir_{cfg.name} = {os.path.join(N.tempdir, N.active_platform, cfg.name)}\n")
			f.write("\n")
...
""" % (sys.executable, N.code_path, N.active_platform))
			for cfg_name in N.configs_ordered:
				...
				if cfg.is_active:
					...
					for v in cfg.copy_files:
						f.write(f"build $outdir_{suff}\\{PathGetFilename(v)}: copy_{suff} {v}\n")
				...
				if cfg.is_active:
					...
					if len(target_names) > 0:
						...
						for tname in target_names:
							f.write(f"{tname}_{cfg_name} ")#// % (GetTargetPath(tname, cfg)))
						f.write("\n")
			f.write("\n")
			...
			f.write(f"default build.ninja {cfg_default.name}\n\n")
